...
import type { QuestionLens, ReactionKind } from "./types";
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === "/api/admin/export" && request.method === "GET") {
        if (!(await isAdmin(request, env.ADMIN_TOKEN_SHA256))) return jsonError("not found", 404);
      ...
      const status = message.includes("limit") ? 429 : message.includes("too large") ? 413 : 400;
      console.error(
...
async function isAdmin(request: Request, expectedHash: string): Promise<boolean> {
